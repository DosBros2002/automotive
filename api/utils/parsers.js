 ...
 */
export function redactMessage(str) {
  if (!str) {
    return "";
  }
 ...
 */
const truncateLongStrings = (value, length = 300) => {
  if (typeof value === "string") {
    return value.length > length
      ? value.substring(0, length) + "... [truncated]"
      : value;
  }
 ...
 */
export const debugTraverse = winston.format.printf(
  ({ level, message, timestamp, ...metadata }) => {
    let msg = `${timestamp} ${level}: ${truncateLongStrings(message?.trim(), 300)}`;
    try {
      if (level !== "debug") {
        return msg;
      }
